...
const CuponesList = () => {
    ...
    const fetchCupones = async () => {
        try {
            const response = await axios.get('http://localhost:4002/api/cupones'); // Cambia el endpoint si es necesario
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingCupón) {
                // Edit cupón
                await axios.put(`http://localhost:4002/api/cupon/${editingCupón.id_cupon}`, cuponData);
                alert('Cupón actualizado');
            } else {
                // Create new cupón
                await axios.post('http://localhost:4002/api/cupon', cuponData);
                alert('Cupón creado');
            }
            fetchCupones();
            closeModal();
    ...
    const confirmDeleteCupón = async () => {
        try {
            ...
            await axios.put(`http://localhost:4002/api/cupon/${cupónToDelete.id_cupon}`, cuponData);
            // await axios.delete(`http://localhost:4002/api/cupon/${cupónToDelete.id_cupon}`);
